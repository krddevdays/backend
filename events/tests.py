...
from .factories import EventFactory, VenueFactory, ZoneFactory, ActivityFactory, SponsorFactory
from .interfaces import ActivityType, WelcomeActivity, SponsorType
...
class EventsTestCase(TestCase):
    ...
    def test_sponsors(self):
        factories = {
            'SPONSOR': SponsorFactory(event=self.event, type=SponsorType.SPONSOR),
            'INFORMATIONAL_PARTNER': SponsorFactory(event=self.event, type=SponsorType.INFORMATIONAL_PARTNER)
        }
        response = self.client.get(reverse('event-sponsors', args=(self.event.id,)))
        self.assertEqual(response.status_code, 200)
        data = response.json()
